  ...
  async executeSchema() {
    ...
    try {
      ...
      if (existingTables.length > 0) {
        logWarning(`Tabelas já existem: ${existingTables.join(', ')}`);
        logInfo('Pulando execução do schema - banco já configurado');
        this.addStep('Execução do Schema', 'PULADO', 'Tabelas já existem no banco');
        return true;
      }
